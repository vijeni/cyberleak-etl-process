...
init(autoreset=True)
bar_format = (
        "{l_bar}" + Fore.YELLOW + "{bar}" + Style.RESET_ALL + "| {n_fmt}/{total_fmt} [{elapsed}<{remaining}, {rate_fmt}]"
)
...
def processar_arquivo(db, caminho_arquivo, separador):
  ...
  with open(caminho_arquivo, 'r', encoding='utf-8') as arquivo:
    ...
    with tqdm(total=quantidade_linhas, desc=Fore.BLUE + "Processando" + Style.RESET_ALL, bar_format=bar_format,
